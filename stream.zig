...
const WriteCmd = struct {
    ...
    res: IO.SendError!usize = undefined,

    fn run(self: *Self, buffer: []const u8) void {
        self.stream.io.send(*Self, self, callback, &self.stream.completion, self.stream.handle, buffer);
    }

    fn callback(self: *Self, _: *IO.Completion, result: IO.SendError!usize) void {
        self.res = result;
        defer self.done = true;
    }

    fn wait(self: *Self) !usize {
        while (!self.done) try self.stream.io.tick();
        return self.res;
    }
};
...
const ReadCmd = struct {
    ...
    res: IO.RecvError!usize = undefined,

    fn run(self: *Self, buffer: []u8) void {
        self.stream.io.recv(*Self, self, callback, &self.stream.completion, self.stream.handle, buffer);
    }

    fn callback(self: *Self, _: *IO.Completion, result: IO.RecvError!usize) void {
        self.res = result;
        defer self.done = true;
    }
...
pub const Stream = struct {
    ...
    // std.net.Stream implementation using tigerbeetle IO
    io: *IO,
